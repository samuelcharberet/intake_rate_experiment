...
plot_irn <- function(data_i, data_g, data_model, data_ic) {
  ...
  long_df <- data_i %>%
    ...
    mutate(
      time = gsub("bodymass_7th_instar_j|_fw", "", time),
  ...
  data_wc_0 <- data.frame(
    wc = data_ic$indiv_water_content,
  ...
  mod <- mgcv::gam(
    assimilation_rate_dw ~ s(ingestion_rate_dw,
      bs = "cs", k =
        3
    ),
    data = data_i
  ...
  mod <- mgcv::gam(
    geometric_mean_growth_dw ~ s(ingestion_rate_fw,
      bs = "cs", k =
        3
    ),
    data = data_i
  ...
  for (i in 1:nb_elements) {
    ...
    food_col <- which(names(data_element) == paste("food_", elements[i],
      sep =
        ""
    ))
    ...
    average_larvae <- mean(data_element[which(data_element$variable == "larvae"), ]$elemental_value,
      na.rm =
        T
    )
    sd_larvae <- sd(data_element[which(data_element$variable == "larvae"), ]$elemental_value,
      na.rm =
        T
    )

    # Frass elemental content
    average_frass <- mean(data_element[which(data_element$variable == "frass"), ]$elemental_value,
      na.rm =
        T
    )
    sd_frass <- sd(data_element[which(data_element$variable == "frass"), ]$elemental_value,
      na.rm =
        T
    )
    data <- data.frame(
